...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def get_current_user(self):
        ...
        if self.get_secure_cookie("user") and self.DBI.get_user_uuid(self.get_secure_cookie("user").decode()):

            # if the user uuid is the same with the one in the cookie
            # which means this is the most recent authenticated device
            if self.DBI.get_user_uuid(self.get_secure_cookie("user").decode())[0] == self.get_secure_cookie("uuid").decode():
